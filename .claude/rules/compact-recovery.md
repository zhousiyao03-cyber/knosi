...
4. **任务流程**：当前模块清单看 `src/components/layout/navigation.ts`；每个非平凡任务完成后需要留档（`docs/changelog/YYYY-MM-DD-<topic>.md`）、E2E 测试通过、conventional commits（`feat(scope):` / `fix(scope):` / `docs(scope):` / `ops(scope):`）。
...
3. 浏览 `src/components/layout/navigation.ts` 了解当前活模块清单
