...
## B1-4 — EXPLAIN QUERY PLAN 体检（不是 N+1，是 index 健康度审计）

### 原计划 vs 实际决定

B1-4 的原计划是"在仓库里找一个 N+1 查询 → 用 EXPLAIN 证明它慢 → 改成批量化 → 再 EXPLAIN 证明修好了"。

**实际做法**：放弃"找 N+1"。改成"**对 18 个高频查询跑一次系统性 EXPLAIN QUERY PLAN 体检**"。原因是我花了半小时翻 routers，发现 Second Brain 里真正意义上的 N+1 **已经被之前的多轮优化修完了**——`learning-notebook.listTopics` 的注释明确写着 "no N+1"，`oss-projects.collectProjectMetaBatch` 是显式的批量函数。强行找一个又小又人造的 N+1 来"修"是作弊——我会学到**假知识**。

体检是更诚实的做法：不预设能找到什么问题，**真实看看 SQLite 对每条查询选了什么执行计划**。

### EXPLAIN QUERY PLAN 术语速查

跑之前先把 SQLite 会输出的几个关键词记下来，否则看不懂结果：

| 输出片段 | 意思 | 好 / 坏 |
|---|---|---|
| `SEARCH <table> USING INDEX <idx> (col=?)` | 用索引定位到 row | 好 |
| `SEARCH <table> USING COVERING INDEX <idx>` | 用索引而且**不用回表**（所有 SELECT 列都在索引里） | **最好** |
| `SCAN <table>` | 全表扫描（没有 WHERE / 或 WHERE 没吃到 index） | 坏 |
| `SCAN <table> USING COVERING INDEX <idx>` | 走完整个索引（通常是 GROUP BY 全表，没有 WHERE） | 中性——只要是 COVERING 就不贵 |
| `USE TEMP B-TREE FOR ORDER BY` | **index 不帮忙排序**，SQLite 在内存里重排一次 | **坏** |
| `USE TEMP B-TREE FOR GROUP BY` | 同上，但是为 GROUP BY 聚合排 | 坏（数据量小时无感）|
| `USE TEMP B-TREE FOR RIGHT PART OF ORDER BY` | 多列 ORDER BY 的第一列吃到 index，后面的列还得内存排 | 中等 |
| `SCALAR SUBQUERY 1` | 标量子查询，会被优化器独立计算 | 中性 |

**最重要的反直觉**：**"SEARCH USING INDEX"不等于"ORDER BY 也用 index"**。索引能帮 WHERE 定位，但想让 ORDER BY 也走 index，**索引的后半段字段必须正好是 ORDER BY 的字段**。否则 SQLite 会先把 WHERE 结果集拿出来，再在内存里排一次——这就是 TEMP B-TREE FOR ORDER BY 的含义。

### 审计脚本

`scripts/learn/b1-explain-audit.mjs`——对 18 个查询各跑一次 `EXPLAIN QUERY PLAN`，把 detail 行打印出来，用一个简单的 classifier 判定好/坏并生成汇总。classifier 的规则：

- 看到 `TEMP B-TREE FOR ORDER BY` → 警告
- 看到 `TEMP B-TREE FOR GROUP BY` → 警告
- 看到裸 `SCAN <table>`（非 COVERING INDEX）→ 警告
- 否则 → OK
...
### 体检结果（18 个查询，8 绿 10 警）

**绿灯**（index 和查询匹配良好）：

| 查询 | 为什么绿 |
|---|---|
| `notes search LIKE` | WHERE user_id 吃到 index，LIKE 部分无法用 index 是 LIKE 的**结构性局限**，不是 index gap |
| `dashboard: count notes` | COVERING INDEX — 全程不回表 |
| `dashboard: count todos done` | 同上 |
| `queue: snapshot group by status` | COVERING INDEX scan — 数 4 个状态就是要走全索引，这是对的 |
| `portfolio_holdings by user` | `(user_id, symbol)` 复合索引正好匹配 WHERE + ORDER BY |
| `activity_sessions daily range (focus)` | `(user_id, started_at)` 复合索引把时间 range 也吃进去了 |
| `note_links backlinks` | 两个 index 串联（target_idx + notes PK）|
| `knowledge_chunks by source` | source_idx 吃到 WHERE |

**警告**（10 个 file-sort，都是同一种病）：

| 查询 | 症状 | 现有 index | 缺什么 |
|---|---|---|---|
| `notes.list` default | TEMP B-TREE FOR ORDER BY | `(user_id)` | `(user_id, updated_at DESC)` |
| `notes.list +folder` | TEMP B-TREE FOR ORDER BY | `(folder_id)` | `(user_id, folder_id, updated_at DESC)` |
| `bookmarks.list` | TEMP B-TREE FOR ORDER BY | `(user_id)` | `(user_id, created_at DESC)` |
| `todos.list +status` | TEMP B-TREE FOR ORDER BY | `(user_id)` | `(user_id, status, created_at DESC)` 或 `(user_id, created_at DESC)` |
| `todos due today` | partial file-sort | `(user_id, due_date, status)` | 第二层 `updated_at` 也得在 index 里 |
| `folders.list` | TEMP B-TREE FOR ORDER BY | `(user_id)` | `(user_id, sort_order ASC)` |
| `queue: pending by queued_at` | TEMP B-TREE FOR ORDER BY | `(status)` | `(status, queued_at)` |
| `queue: claimNextJob subquery` | 同上（内层是同一条 SELECT）| 同上 | 同上 |
| `learning_notes group by topic` | TEMP B-TREE FOR GROUP BY | `(user_id)` | `(user_id, topic_id)` |
| `token_usage_entries` | TEMP B-TREE FOR ORDER BY | `(user_id)` | `(user_id, created_at DESC)` |

### 病根：所有警告都是同一种模式

10 个警告本质上是 **"index 覆盖 WHERE 但没覆盖 ORDER BY"**。治法是同一个：**扩展成复合索引，把 ORDER BY 的列追加到后面**。

这种扩展遵循一个经典规则——**"equality first, range/sort last"**：

- 用 `=` 比较的列（像 `user_id = ?`）放复合索引**前面**
- 用 range / ORDER BY 的列放**后面**

举例：`notes.list` 是 `WHERE user_id = ? ORDER BY updated_at DESC`——user_id 是等值，updated_at 是排序。所以应该建 `(user_id, updated_at DESC)`。反过来建 `(updated_at, user_id)` 就废了——定位一个用户的笔记得扫整个 index。

对 `queue` 的 claimNextJob：WHERE 是 `status = 'pending' AND queued_at <= ?`，ORDER BY 是 `queued_at ASC`。status 是等值，queued_at 是 range **同时是**排序——所以 `(status, queued_at)` 正好一箭双雕。

### 踩坑记录

1. **我的 classifier regex 一开始把 `SCAN knowledge_index_jobs USING COVERING INDEX ...` 误判为"full table scan"**。原因是 `\b SCAN \s+ \w+ (?!\s+USING\s+INDEX)` 的负前瞻会 backtrack——regex 引擎把 `knowledge_index_jobs` 匹配成 `knowledge_index_job`，然后负前瞻看到 `s USING...` 不是 `USING`，于是整体匹配成功。对策：**先用字符串替换把所有 `SCAN ... USING [COVERING] INDEX ...` 片段移除，再检查还剩没剩 SCAN**。比写负前瞻正则可靠多了。这是"正则不要过度工程化"的又一次教训——能用两步字符串操作代替的复杂正则就别用正则。

2. **"SCAN" 不一定是坏的**。`queue.snapshot` 的 `SELECT status, count(*) GROUP BY status` 走 `SCAN USING COVERING INDEX` 是**正确**的执行计划——因为查询本身就是"数全表分组"，没有 WHERE 给 index 定位，只能 walk 整个 index。而且 COVERING INDEX 比真表窄得多，代价小。别被"SCAN"这个词吓到。

3. **LIKE 走不走 index**。`notes search LIKE` 只能吃到 `user_id` 那个 index，LIKE 的部分 SQLite 无能为力。**LIKE 的前缀匹配**（如 `'foo%'`）能走 index，但 `'%foo%'` 这种包含式不行——需要 FTS（Full-Text Search）才能加速。B1 的作用域内不处理，但记下这是 Second Brain 将来如果要做全文搜索的一个路径。

### 验证

- `pnpm build` — 脚本是纯 node，不影响 Next build，略
- ESLint — 同上
- **Audit 脚本跑通**：18 查询，8 绿 10 警，结果稳定可重现
- 无 production 影响：脚本只读，没有任何写入

### 学到了什么（after state）

1. **EXPLAIN QUERY PLAN 是后端日常工具，不是救火专用**。以前我只在"某个 API 慢到崩了"时才想起来跑 EXPLAIN。这次对 18 个查询**无差别**跑一遍，在 5 分钟内暴露了 10 个潜在问题——这是一种低成本的例行体检。以后新加 query 时应该顺手跑一次，不是等出 bug 才查。
2. **"index 能帮 WHERE 不代表能帮 ORDER BY"**——这是我今天最大的认知修正。以前我看到 `SEARCH USING INDEX` 就以为"这个查询走 index 了，没事"，但 `TEMP B-TREE FOR ORDER BY` 说明 SQLite 还额外做了一次排序工作。要让 index **也**帮到 ORDER BY，必须把排序字段放进 index 后半段。
3. **"equality first, range/sort last"** 是复合索引的字段顺序黄金规则。以后设计 index 先问两个问题：(a) WHERE 里哪些是 `=`？(b) 剩下哪些是 range 或 ORDER BY？前者放前面，后者放后面。
4. **真实的问题比人造的问题更有教学价值**。这次不预设 N+1 能找到，反而发现 10 个 file-sort——这些都是正在发生的、每次打开列表页就会跑的真实痛点。如果当初硬塞一个人造 N+1 去修，这 10 个问题会继续藏着。
5. **Regex 负前瞻的 backtrack 是隐形地雷**。写 classifier 这种轻量工具时，**先用字符串替换删除已知模式，再做简单判断** 是更安全的做法——不用和正则引擎斗智斗勇。

### 下一步

B1-5 就是基于这 10 个 warn 修 index。**但不是 10 条都修**——有些 table 数据量太小（`learning_notes` 现有 <50 条，`folders` <20 条），file-sort 的绝对耗时完全无感；花一条 index 的 schema 复杂度换几微秒是亏的。B1-5 会基于"数据量 × 访问频率"筛出优先修哪几个，再跑一次 audit 脚本作为 before/after 的对比。
