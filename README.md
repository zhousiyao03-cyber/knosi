...
- **Curriculum Map** — `/learn/map` knowledge gap analysis across target roles (AI Engineer, Backend Architect). ~240 seeded topics with 4-state mastery (blank/heard/learning/mastered). Auto-links existing learning **and** general notes by string matching; click a topic to open a side panel with mastery toggle, linked notes (with source badges + bulk unlink), AI description generation, inline rename/delete, and quick "Create new note". Editable in-place: add new tracks/areas/topics. Note editors (both kinds) surface curriculum tags inline; deep-link from a tag jumps back to the map and locates the topic.
- **Knowledge Audit** — `/learn/audit` runs an LLM gap analysis on a chosen track, producing strengths / weak areas / missing must-knows / next steps. Past audits stored for comparison. Embedded 90-day mastery progress chart shows mastered/learning/heard counts over time.
